...
class TaskManagement:
    ...
    def extract_data(self, sleep_timer, retry=0, **kwargs):
        """
        ...
        if retry >= 5:
            return
        headers = {"Authorization": API_KEY}  # env
        ...
        return self.extract_data(sleep_timer, retry + 1, **kwargs)

    def get_records(self, start_page, end_page, query, records_per_page, sleep_timer):
        """
...
def load_into_db(db_file, number_of_records):
    ...
    """
    t = TaskManagement()
    t.get_records(1, 4, 'nature', 40, 1)
    f = open('database_ddl.sql', 'r')
...
load_into_db("500rows.sqlite3", 40)
